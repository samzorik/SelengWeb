...
public class MatrixPanel
  ...
  protected JButton m_updateBt = new JButton("Обновить");

  /** The button to display a window to select attributes */
  protected JButton m_selAttrib = new JButton("Выбрать атрибуты");
  ...
  protected JButton m_resampleBt =  new JButton("Масштаб % :");
  ...
  private final JLabel m_plotSizeLb = new JLabel("Размер графиков: [100]");

  /** Displays the current size beside the slider bar for point size */
  private final JLabel m_pointSizeLb = new JLabel("Размер точек: [10]");
  ...
  protected JCheckBox m_fastScroll = 
    new JCheckBox("Быстрая прокрутка");
  ...
  public MatrixPanel() {
	...
	public void stateChanged(ChangeEvent ce) {
	  m_plotSizeLb.setText("Размер графиков: ["+m_plotSize.getValue()+"]");
	...
	public void stateChanged(ChangeEvent ce) {
	  m_pointSizeLb.setText("Размер точек: ["+m_pointSize.getValue()+"]");
    ...
    m_pointSizeLb.setText("Размер точек: [1]");
    m_pointSizeLb.setPreferredSize( m_pointLBSizeD );
    ...
    gbc.weightx=0; gbc.gridwidth = GridBagConstraints.RELATIVE;
    p4.add( new JLabel("Детализация: "), gbc);
    gbc.weightx=1; gbc.gridwidth = GridBagConstraints.REMAINDER;
    p4.add(m_jitter, gbc);
...
//    p3.add(m_fastScroll, gbc);
    ...
    p2.setBorder(BorderFactory.createTitledBorder("Цвет классов"));
    ...
    final JLabel lb = new JLabel("Матрица\n графиков");
  ...
  */
  private class Plot
	...
	return;
      ...
      JFrame jf = new JFrame("Модуль кластеризации: Визуализация "+m_data.relationName() );
