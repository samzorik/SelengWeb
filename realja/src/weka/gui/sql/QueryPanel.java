...
public class QueryPanel 
  ...
  protected WHExtractor whex = new WHExtractor();
  /** the execute button. */
  protected JButton m_ButtonExecute = new JButton("Выполнить запрос");

  /** the clear button. */
  protected JButton m_ButtonClear = new JButton("Очистить");

  protected JButton m_ButtonBuildQuery = new JButton("Выбрать измерения");
  /** the history button. */
  protected JButton m_ButtonHistory = new JButton("История...");
  ...
  protected void createPanel() {
    ...
    panel.add(m_ButtonBuildQuery, BorderLayout.SOUTH);
    panel2 = new JPanel(new BorderLayout());
    panel.add(panel2, BorderLayout.CENTER);
    
    m_ButtonBuildQuery.addActionListener(new ActionListener() {
	public void actionPerformed(ActionEvent e) {
		whex.run();
	}
     });
