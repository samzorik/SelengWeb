 ...
 * @see Drawable
 */
public class CobwebModification 
...
//      m_clusterInstances.instance(index)
      if (m_children == null) {
	text.append("\n");
	for (int j = 0; j < depth; j++) {
	  text.append("|   ");
	}
	text.append("лист "+m_clusterNum+" ["
     ...
     */
    public class struct{
    	public int num_attr,attr_value;
    	public double perc;
    	struct(int n_a,int a_v,double p)
    	{
    		num_attr=n_a;
    		attr_value=a_v;
    		perc=p;
    ...
    public String SplitStr(String text,int size)
    {
    	...
    	while (index != length) {  // Пока не дойдём до последнего элемента
    	  if (cnt + arrWords[index].length() <= size) { // Если текущая фраза + текущее слово в массиве arrWords не превышает 30
    ...
    protected void graphTree(StringBuffer text) throws Exception {
      int num_inst=m_clusterInstances.numInstances();
      if(m_children!=null)
      {
    	  text.append("N"+m_clusterNum
    			  ...
    			  +m_clusterNum+" ("
    			  +m_clusterInstances.numInstances()+")");
    	  ArrayList<struct> good_attr=new ArrayList();
//    	  HashMap <Integer, Double> devs = new HashMap();
    	  ArrayList<Pair<Integer,Double>> low = new ArrayList(), medium= new ArrayList(), high = new ArrayList();
    	  for (int i = 0; i < m_clusterInstances.numAttributes(); i++) {
    		  if (m_clusterInstances.attribute(i).isNominal())
    		  {
				for (int j = 0; j < m_attStats[i].nominalCounts.length; j++) {
					if ((double)m_attStats[i].nominalCounts[j]/(double)num_inst>0.7)
					{
						text.append("\\n"+m_clusterInstances.attribute(i).name()+" = "
								+m_clusterInstances.attribute(i).value(j)
								+" "+(double)m_attStats[i].nominalCounts[j]/(double)num_inst*100.0+" %");
	//					good_attr.add(new struct(i,j,(double)m_attStats[i].nominalCounts[j]/(double)num_inst));
					}
				}
    		  }
    		  else
    		  if (m_clusterInstances.attribute(i).isNumeric())
    		  {
    			  m_attStats[i].numericStats.calculateDerived();
    			  if (m_attStats[i].numericStats.stdDev<0.25)
    			  {
    				  if ( m_attStats[i].numericStats.mean < -0.3)
    				  {
    					  low.add(new Pair(i, m_attStats[i].numericStats.mean));
    				  }
    				  else
    				  if ( m_attStats[i].numericStats.mean < 0.3)
    				  {
    					  medium.add(new Pair(i, m_attStats[i].numericStats.mean));
    				  }
    				  else   					  
   					  {
   						  high.add(new Pair(i, m_attStats[i].numericStats.mean));
   					  }
//    				  devs.put(i, m_attStats[i].numericStats.stdDev);
//    				  text.append("\\n Ст. отклон атрибута "+m_clusterInstances.attribute(i).name()+" = "
//    						  +m_attStats[i].numericStats.stdDev);
    	  ...
    	  if (low.size()>0)
    	  {
    				  ...
    				  for (int i = 0; i < low.size(); i++) {
						text.append(SplitStr(m_clusterInstances.attribute(low.get(i).getFirst()).name(),40));
    				  }
    	  }
    	  if (medium.size()>0)
    	  {
    		  ...
    		  for (int i = 0; i < medium.size(); i++) {
    			  text.append(SplitStr(m_clusterInstances.attribute(medium.get(i).getFirst()).name(),40));
    		  }
    	  }
    	  if (high.size()>0)
    	  {
    		  ...
    		  for (int i = 0; i < high.size(); i++) {
    			  text.append(SplitStr(m_clusterInstances.attribute(high.get(i).getFirst()).name(),40));
    		  }
    	  }
    	  text.append("\"]\n");
      }
      else
      if((m_children == null) && (m_clusterInstances.numInstances()>1))
      {
    	  text.append("N"+m_clusterNum
    			  + " [label=\""+"Кластер "
    			  +m_clusterNum);//+" "+m_clusterInstances.instance(0).stringValue(0));
    	  for (int i = 0; i < m_clusterInstances.numInstances(); i++) {
    						  text.append( "\\n"+m_clusterInstances.instance(i).stringValue(0));
    	  }
    	  text.append("\" "
    	  +((m_children == null) 
      ...
      else
      {
    	  text.append("N"+m_clusterNum
    			  + " [label=\""+"лист "
    					  +m_clusterNum+" "
    					  +" ("+((m_clusterInstances.numInstances()==1)?
    							  m_clusterInstances.instance(0).stringValue(0):
    								  m_clusterInstances.numInstances())
    								  +")\" "
    								  +((m_children == null) 
    										  ? "shape=box style=filled " : "")
   ...
   */
  protected double m_cutoff = 0.01 * CobwebModification.m_normal;
...
//      host.updateStats(instance, true);
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (optionString.length() != 0) {
      ...
      m_cutoff = 0.01 * CobwebModification.m_normal;
    }
  ...
  public static void main(String[] argv) {
    runClusterer(new CobwebModification(), argv);
  }
